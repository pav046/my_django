...
class GetPagesTestCase(TestCase):
    fixtures = ['women_women.json', 'women_category.json', 'women_tagpost.json', 'women_husband.json']
    ...
    def test_data_mainpage(self):
        ...
        path = reverse('home')
        response = self.client.get(path)
        self.assertQuerySetEqual(response.context_data['post'], w[:5])
    ...
    def test_paginate_mainpage(self):
        ...
        w = Women.published.all().select_related('cat')
        self.assertQuerySetEqual(response.context_data['post'], w[(page - 1) * paginate_by:page * paginate_by])
